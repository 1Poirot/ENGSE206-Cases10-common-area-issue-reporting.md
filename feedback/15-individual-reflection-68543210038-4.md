# 15 — Individual Reflection

> ให้สมาชิกแต่ละคนคัดลอก template นี้เป็นไฟล์ของตนเอง เช่น `15-individual-reflection-student-id.md`

## Student Information

| Field | Detail |
|---|---|
| Student ID | 68543210038-4 |
| Name | Phumipat Wongdaw |
| Primary Role(s) | UX & Evidence Lead |

## 1. My Contribution

อธิบายสิ่งที่ทำจริง พร้อมอ้างอิง file/commit/evidence

| อธิบายสิ่งที่ทำจริง                           | อ้างอิง file/commit/evidence |                                                                                                            |
| ------------------------------ | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The information has been entered into the 00-Project Profile. | docs/00-project-profile.md |
| project-management & feedback | feedback/week-02-peer-feedback.md & project-management/week-02-week-03-worklog.md |
| 03-interview-guide.md | docs/03-interview-guide.md |
| update 03-interview-guide.md | docs/03-interview-guide.md |
| 04-negotiation-record | docs/04-negotiation-record.md |
| update Status | docs/05-requirement-backlog.md |
| update Status | docs/05-requirement-backlog.md |
| upload 05-prioritization-rationale.md | docs/05-prioritization-rationale.md |
| update Requirement Backlog | docs/05-requirement-backlog.md |
| PeerCross-Review | evidence/week-05/PeerCross-Review.md |
| Update validation traceability and individual reflection with contributions and learnings | docs/08-validation-traceability.md, feedback/15-individual-reflection-68543210038-4.md, project-management/team-worklog.md |
| docs(06): add sections 1-2 | docs/06-requirement-models.md |
| docs(06): update roles in user stories and acceptance criteria | docs/06-requirement-models.md |

## 2. What I Learned About Requirements and Design

[ได้รู้จักการเเยกความต้องการของผู้ใช้ระบบว่าจริงๆเเล้วเขาต้องการอะไรกันเเน่]

## 3. A Decision I Influenced

[ช่วยในการตรวจเช็คเนื้อหาไฟล์ที่ได้เเก้ไขต่างๆ]

## 4. Feedback I Received and How I Responded

[การตอบคำถามที่ยังกว้างเกินไป เเก้ไขโดยการบีบคำตอบให้เเคบกว่าเดิมเพื่อให้ตรงกับข้อมูลของข้อนั้นจริงๆ]

## 5. What I Would Improve Next Time

[การตอบคำถามให้ตรงประเด็นเเยกเเยะได้ไม่กำกวม]

## 6. Evidence Links

- [commit/file/evidence link] <br>
https://github.com/1Poirot/ENGSE206-Cases10-common-area-issue-reporting.md/commits/main/?author=Phumipat-2005